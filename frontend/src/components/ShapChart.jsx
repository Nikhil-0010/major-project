...
export default function ShapChart({ features, values }) {
  ...
  const leftMargin = isMobile ? 20 : 90;
  const yAxisWidth = isMobile ? 70 : 85;
  const fontSize = isMobile ? 9 : 10;

  return (
    <div className="shap-chart">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} layout="vertical" margin={{ left: leftMargin, right: 16, top: 4, bottom: 4 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(15,23,42,0.06)" />
          <XAxis type="number" tick={{ fill: "#64748b", fontSize }} axisLine={{ stroke: "rgba(15,23,42,0.12)" }} />
          <YAxis type="category" dataKey="name" tick={{ fill: "#334155", fontSize }} axisLine={false} width={yAxisWidth} />
          <Tooltip
          ...
          <Bar dataKey="value" radius={[0, 3, 3, 0]} barSize={isMobile ? 12 : 16}>
            {data.map((e, i) => (
